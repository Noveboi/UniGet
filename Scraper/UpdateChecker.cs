...
namespace Scraper
{
    public class UpdateChecker
    {
        ...
        /// <returns>The documents that have been added</returns>
        public DocumentCollection GetSubjectUpdates(Subject before, Subject after)
        {
            if (before != after)
                throw new Exception("The 'before' subject does not match the 'after' subject");
            AppLogger.WriteLine($"Checking for updates in subject: {before}");
            ...
            watch.Stop();
            AppLogger.WriteLine($"{updates.BaseCount} updates found! Searched in {watch.ElapsedMilliseconds}ms");
            return updates;
        }
        ...
        private DocumentCollection GetFolderUpdates(Folder oldFolder, Folder newFolder, DocumentCollection documents)
        {
            ...
            foreach (var doc in newFolder.Documents.Files)
            {
                ...
                {
                    AppLogger.WriteLine($"\t\tDetected added document: {doc}");
                ...
                {
                    AppLogger.WriteLine($"\t\tDetected modified document: {doc}");
                    documents.Files.Add(doc);
                }   
            }
            ...
            foreach (var folder in newFolder.Documents.Folders)
            {
                ...
                {
                    AppLogger.WriteLine($"\tSearching in {old.Name}");
                    GetFolderUpdates(old, folder, documents);
                }
                else
                {
                    AppLogger.WriteLine($"\t\tDetected added folder: {folder}");
            ...
            foreach (var doc in oldFolder.Documents.Files)
            {
                ...
                {
                    AppLogger.WriteLine($"\t\tDetected removed document: {doc}");
                }
            }
            // DETECTS: Removal
            ...
            foreach (var folder in oldFolder.Documents.Folders)
            {
                ...
                {
                    AppLogger.WriteLine($"\t\tDetected removed folder: {folder}");
                }
            }
